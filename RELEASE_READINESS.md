...
| Mobile lint | Done | `npm run lint` passes with ESLint/Expo config. | None for current code. |
| Mobile unit tests | Done | `npm test` validates MANAS catalog counts, required topics, booking state transitions, assistant copy, native-module startup guards, and release config reporting. | Add UI automation when device farm is available. |
...
| Android launch | Partial | A prior Android debug launch opened MANAS without fatal MANAS `AndroidRuntime` or `ReactNativeJS` lines. In the latest verification pass, `adb devices` reported no attached emulator/device, so live launch could not be repeated. | Run the documented ADB smoke command on an attached emulator/device before release. |
| Release backend/Firebase config | Missing | `JAI_REQUIRE_RELEASE_BACKEND_FIRST_CONFIG=1 npm run release:verify-backend-first` lists missing Firebase public env vars exactly. | Provide production Firebase public config before calling release ready. |
...
| Select category, topic, coach, date/time, and book demo session | Done | Home category cards route to topic lists; topic details route to coach list; booking screen selects calendar date, slot, and session type; backend `POST /sessions` creates confirmed demo sessions; mobile now has local MANAS fallback data and a confirmation screen for demo mode. | Needs end-to-end QA with real users and production auth. |
...
| Booking confirmations | Partial | Backend creates a `BOOKING_CONFIRMED` notification; mobile shows a confirmation screen and labels mock notification behavior as an MVP placeholder. | Production push/email/SMS delivery is not validated. |
