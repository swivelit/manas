...
### Jitsi Android compatibility

`@jitsi/react-native-sdk` 13.1.1 uses the legacy root extra property
`gradlePluginVersion` while its Android Gradle script is evaluated. Expo SDK 56
does not generate that property, so `./plugins/with-jitsi-android-compat.js`
derives the actual AGP version from React Native's version catalog and injects
the marked root-project property during every clean prebuild. It also removes
the unused WebRTC media-projection service for the v1 release policy.

`mobile/android` is generated by Expo CNG. Do not hand-edit generated Gradle or
manifest files; update the config plugin or `mobile/app.json` and regenerate
with `npx expo prebuild --platform android --clean` instead.

The existing `mobile/scripts/patch-android-gradle-node.mjs` postinstall helper
also reapplies the checked-in Kotlin compatibility patches for Jitsi's
`react-native-splash-view` and gesture-handler peers after every `npm ci`.
...
support are disabled. Android screen sharing is not a MANAS v1 feature, so the
Jitsi `desktop` action is not exposed to coaches. Consequently, the current
release must not declare foreground-service permissions or services, and no Play
Console foreground-service declaration should be required.
